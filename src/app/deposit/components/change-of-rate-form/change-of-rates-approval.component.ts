...
@Component({
  selector: 'app-change-of-rates-approval',
  templateUrl: './change-of-rates-approval.component.html',
  styleUrls: []
})
export class ChangeOfRatesApprovalComponent implements OnInit {
  ...
  submitApproval(ItemDetails: any) {
    let body = {
      targetId: parseInt(ItemDetails.changeOfRateId),
    ...
    swal
      ...
      .then(result => {
        if (result.value) {
          ...
          return this.DepositAccountService.submitStaffDepsoitChangeOfRateApproval(body).subscribe(
            res => {
  ...
  getChangeOfRates() {
    this.loadingService.show();
    return this.DepositAccountService.getAllChangeOfRatesAwaitingApprovals().subscribe(
      data => {
        this.ChangeOfRates = data.changeOfRates;
        this.loadingService.hide();
      },
      err => {
        this.loadingService.hide();
