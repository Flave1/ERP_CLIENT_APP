...
@Component({
  selector: "app-treasury-investment-appraisal",
  templateUrl: "./treasury-investment-appraisal.component.html",
  styleUrls: ["./treasury-investment-appraisal.component.css"]
})
export class TreasuryInvestmentAppraisalComponent implements OnInit {
  ...
  getAppraisals() {
    ...
    this.treasuryService.getInvestmentAppraisal().subscribe(
      data => {
        this.loadingService.hide();
        if (data.issuerInvestments !== null) {
          this.investments = data.issuerInvestments;
        } else {
          this.investments = []
        }
      },
  ...
  getApprovalTrail(loanApplicationId, operationId) {
    this.creditAppraisalService
      ...
      .subscribe(data => {
        ...
        // this.actualdate = data.aprovalDetails[0].arrivalDate;
      });
  }
  ...
  withinApprovalLimits() {
    ...
    if (
      ...
      this.selectedItem.approvedAmount
    ) {
      return true;
    }
    return false;
  ...
  submitApproval(formObj) {
    let body = {
      targetId: this.treasureIssuerInvestmentId,
      ...
      .then(result => {
        if (result.value) {
          ...
          __this.treasuryService
            .submitInvestmentApproval(body)
            .subscribe(data => {
              __this.loadingService.hide();
              const message = data.status.message.friendlyMessage;
              if (data.status.isSuccessful) {
                swal.fire("GOS FINANCIAL", message, "success");
                __this.getAppraisals();
  ...
  onSelectClicked(row) {
    ...
    this.getApprovalTrail(this.treasureIssuerInvestmentId, this.workFlowToken);
    //
    // this.getCustomerRatioCalculation(this.customerId);
    // this.getCustomerFSRatioValue(this.customerId);
  }
