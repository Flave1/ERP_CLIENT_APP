...
export class PlReportComponent implements OnInit {
    ...
    getFinancialYear() {
        return this.financialYearService.getAllFinacialYearByStatus().subscribe(data => {
    ...
    processData(form) {
        ...
        let path: string = `${environment.report_url}/Reporter/ProfitandLossReport?date1=${this.date1}&date2=${this.date2}`;
      this.reportSrc = this.sanitizer.bypassSecurityTrustResourceUrl(
        path
      );
      this.displayTestReport = true;
        // this.loadingService.show();
        // this.reportService.getPlReport(this.date1, this.date2).subscribe(
        //     data => {
        //         this.loadingService.hide();
        //         if (data["result"] != null) {
        //
