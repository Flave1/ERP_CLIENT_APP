...
@Component({
  selector: "app-treasury-product-type-list",
  templateUrl: "./treasury-product-type-list.component.html",
  styleUrls: ["./treasury-product-type-list.component.css"]
})
export class TreasuryProductTypeListComponent implements OnInit {
  ...
  showAddNew() {
    this.router.navigate(["/treasury/product-type-setup"]);
  }
  // onRowSelect(event) {
  ...
  getAllProduct() {
    ...
    this.treasuryService.getAllProductTypes().subscribe(
      data => {
        ...
        this.productInformation = data.productTypes;
      },
      err => {
  ...
  exportProductInformation() {
    this.loadingService.show();
    this.treasuryService.exportProductType().subscribe(
      response => {
        this.loadingService.hide();
        ...
        if (data != undefined) {
          ...
          var bb = new Blob([ab]);
          try {
            var file = new File([bb], "Treasury Product Type.xlsx", {
  ...
  multipleDelete() {
    ...
    if (tempData !== undefined) {
      tempData.forEach(el => {
        ...
        // };
        targetIds.push(el.productTypeId);
      });
    }
    ...
    swal
      ...
      .then(result => {
        if (result.value) {
          this.loadingService.show();
          return this.treasuryService.multipleDeleteProductType(body).subscribe(
            res => {
              this.loadingService.hide();
