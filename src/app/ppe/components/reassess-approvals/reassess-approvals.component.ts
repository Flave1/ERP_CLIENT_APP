...
@Component({
  selector: "app-reassess-approvals",
  templateUrl: "./reassess-approvals.component.html",
  styleUrls: ["./reassess-approvals.component.css"]
})
export class ReassessApprovalsComponent implements OnInit {
  activeIndex: any;
  ...
  getAwaitingApprovals() {
    ...
    return this.ppeService.getReassessmentAppraisals().subscribe(
      data => {
  ...
  submitApproval(details: any) {
    ...
    swal
      ...
      .then(result => {
        if (result.value) {
          this.loadingService.show();
          return this.ppeService.submitReassessmentApproval(body).subscribe(
            res => {
  ...
  multiApprove() {
    ...
    swal
      .fire({
        title: "Are you sure you want to approve records?",
        text: "You won't be able to revert this",
        icon: "warning",
        showCancelButton: true,
        confirmButtonText: "Yes!"
      ...
      .then(res => {
        if (res.value) {
          this.loadingService.show();
          return this.ppeService.multiApproveReassessment(payload).subscribe(
            res => {
              this.loadingService.hide();
              const message = res.status.message.friendlyMessage;
              if (res.status.isSuccessful) {
